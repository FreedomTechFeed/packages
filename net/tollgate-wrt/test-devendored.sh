...
#      been absent since the pre15 pin; still absent at the current pin 2796d96c,
#      which pre16 and pre17 share) --
...
# Module #517 DELETED that file, so at the pre15 pin (42496214) and at the
# current pin (2796d96c, pre16 and pre17) the tarball does
...
if grep -q 'PKG_TARBALL_DIR)/packaging/files/tollgate-captive-portal-site/welcome.html' "$FEED/Makefile"; then
    fail "Gate D: Makefile installs welcome.html from \$(PKG_TARBALL_DIR) again, but neither the pre15 (42496214) nor the current (2796d96c) pin ships it (guaranteed package-build failure)"
    DEAD_WELCOME=1
fi
